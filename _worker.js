// ===============================================
// Cloudflare Worker (Pages連携 最終版)
// 正しい `onRequest` 構文を使用した最終コード
// ===============================================

// --- このセクションのヘルパー関数は変更ありません ---
const jwt = {
...
async function getValidAccessToken(env, userId) {
    ...
    if (!tokens || !tokens.refresh_token) throw new Error("Googleアカウントと連携されていません。再度連携してください。");
    if (!tokens.expires_at || tokens.expires_at < Date.now() + 5 * 60 * 1000) {
        ...
        if (!tokenResponse.ok) {
            ...
            throw new Error("Google認証トークンのリフレッシュに失敗しました。連携が解除された可能性があります。再度連携してください。");
        }
...
async function googleApiRequest(accessToken, url, options = {}) {
    ...
    if (!response.ok) {
        ...
        throw new Error(error.error.message || 'Google APIリクエストに失敗しました。');
    }
...
// ===============================================
// ★★★ ここが根本的な修正点です ★★★
// `export default { fetch }` ではなく、
// `export const onRequest` という単一の関数をエクスポートします。
// ===============================================
...
export async function onRequest(context) {
  // contextから、必要なオブジェクトを分割代入で取り出します
  const { request, env, next } = context;
  const url = new URL(request.url);

  // 1. APIリクエスト（/api/で始まる）の場合、このWorkerが処理します
  if (url.pathname.startsWith('/api/')) {
    ...
    try {
      if (url.pathname.startsWith("/api/auth")) {
        ...
        if (url.pathname === "/api/auth/google/redirect-url" && request.method === "GET") {
            const authResult = await authMiddleware(request, env);
            if (authResult.error) return jsonResponse(authResult, authResult.status);
            const googleUrl = new URL("https://accounts.google.com/o/oauth2/v2/auth");
            googleUrl.searchParams.set('client_id', env.GOOGLE_CLIENT_ID);
            googleUrl.searchParams.set('redirect_uri', `${env.ROOT_URL}/api/auth/google/callback`);
        ...
        if (url.pathname === "/api/auth/google/callback" && request.method === "GET") {
            ...
            if (!code || !state) return jsonResponse({ error: 'Authorization code or state is missing.' }, 400);
            try {
                ...
                const tokenResponse = await fetch("https://oauth2.googleapis.com/token", {
                    ...
                    body: JSON.stringify({
                        ...
                        redirect_uri: `${env.ROOT_URL}/api/auth/google/callback`, grant_type: 'authorization_code'
                    })
                });
                ...
                await saveGoogleTokens(env, userId, tokens);
                return Response.redirect(`${env.ROOT_URL}?google_auth_success=true`, 302);
            } catch (e) {
                console.error("Google auth callback error:", e);
                return jsonResponse({ error: 'Invalid state or token exchange failed.'}, 400);
            }
        }
      }
      ...
      if (url.pathname === "/api/auth/google/disconnect" && request.method === "POST") {
          await env.PLINY_KV.delete(`google_token:${userId}`);
          return jsonResponse({ success: true, message: "Google Calendar integration disconnected." });
      }
      ...
      if (url.pathname === "/api/data" && (request.method === "PUT" || request.method === "PATCH")) {
        const body = await request.json();
        const { expectedVersion } = body;
        const data = await getUserData(env, userId);
        const { tasks: currentTasks, labels: currentLabels, version: currentVersion } = data;
        ...
        await env.PLINY_KV.put(`data:${userId}`, JSON.stringify({
            tasks: newTasks, labels: newLabels, version: newVersion,
        }));
      ...
      if (url.pathname === "/api/sync/google" && request.method === "POST") {
          ...
          const eventListResponse = await googleApiRequest(accessToken, `https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events?privateExtendedProperty=plinyTaskId`);
          const googleEvents = eventListResponse.items || [];
          const googleEventMap = new Map(googleEvents.map(e => [e.extendedProperties.private.plinyTaskId, e]));
          ...
          for (const event of googleEvents) {
              ...
              if (!plinyTask) {
                  ...
                  const plinyUpdated = plinyTask.googleSyncTimestamp ? new Date(plinyTask.googleSyncTimestamp) : new Date(0);
                  if (eventUpdated > plinyUpdated) {
                      plinyTaskMap.set(plinyTaskId, googleEventToPlinyTask(event, plinyTask));
                      hasChanges = true;
                  }
              }
          }
          for (const task of plinyTaskMap.values()) {
              ...
              if (!googleEvent) {
                  const newEvent = await googleApiRequest(accessToken, `https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events`, {
                      method: 'POST', body: JSON.stringify(plinyTaskToGoogleEvent(task))
                  });
                  task.googleEventId = newEvent.id;
                  hasChanges = true;
              } else {
                  const plinyUpdated = task.googleSyncTimestamp ? new Date(task.googleSyncTimestamp) : new Date(0);
                  const eventUpdated = new Date(googleEvent.updated);
                  if (plinyUpdated > eventUpdated) {
                      await googleApiRequest(accessToken, `https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events/${googleEvent.id}`, {
                          method: 'PUT', body: JSON.stringify(plinyTaskToGoogleEvent(task))
                      });
                  }
              }
          ...
          for (const task of tasks) {
              if (!plinyTaskMap.has(task.id) && task.googleEventId) {
                  await googleApiRequest(accessToken, `https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events/${task.googleEventId}`, { method: 'DELETE' }).catch(e => console.warn(e));
                  hasChanges = true;
              }
          }
          const finalTasks = Array.from(plinyTaskMap.values());
          const newVersion = version + (hasChanges ? 1 : 0);
          await env.PLINY_KV.put(`data:${userId}`, JSON.stringify({ tasks: finalTasks, labels, version: newVersion }));
          return jsonResponse({
              tasks: finalTasks, labels, version: newVersion,
              isGoogleConnected: true, message: "Sync completed successfully"
          });
      }
  ...
  // 2. APIリクエストでなければ、リクエストをCloudflare Pagesの静的アセットハンドラに渡します。
  // これが index.html を探しに行くための「パス」の役割を担います。
  return next();
}