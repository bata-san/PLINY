...
const modalTaskText = labelModal ? document.getElementById('modal-task-text') : null;
const modalLabelsContainer = labelModal ? document.getElementById('modal-labels-container') : null;
const modalSaveBtn = labelModal ? document.getElementById('modal-save-btn') : null;
const modalCancelBtn = labelModal ? document.getElementById('modal-cancel-btn') : null;
const modalCloseBtn = labelModal ? labelModal.querySelector('.modal-close-btn') : null;
const parentModal = document.getElementById('parent-selector-modal');
const parentModalTaskText = parentModal ? document.getElementById('parent-modal-task-text') : null;
const parentTaskSelect = parentModal ? document.getElementById('parent-task-select') : null;
const parentModalSaveBtn = parentModal ? document.getElementById('parent-modal-save-btn') : null;
const parentModalCancelBtn = parentModal ? document.getElementById('parent-modal-cancel-btn') : null;
const parentModalCloseBtn = parentModal ? parentModal.querySelector('.modal-close-btn') : null;
...
const getHighestPriorityLabel = (task) => { if (!task || !task.labelIds || task.labelIds.length === 0) return null; return task.labelIds.map(id => labels.find(l => l.id === id)).filter(Boolean).sort((a, b) => a.priority - b.priority)[0]; };
...
// --- 初期化処理 ---
const init = () => {
    // ▼▼▼ 修正箇所: Undoバーをbody直下に生成する処理を復活 ▼▼▼
    if (!document.getElementById('undo-bar')) {
        const undoBar = document.createElement('div');
        undoBar.id = 'undo-bar';
        // スタイルをJSで直接設定して、CSSの依存を減らし確実性を上げる
...
// (initializeCalendar, loadData, saveData, renderAll, renderTaskList, renderCalendar, renderLabelEditor, createLabelItem, renderColorPalette, openLabelModal, closeLabelModal, openParentModal, closeParentModal は変更なし)
const initializeCalendar = () => { calendar = new FullCalendar.Calendar(calendarContainer, { initialView: window.innerWidth < 768 ? 'listWeek' : 'dayGridMonth', locale: 'ja', headerToolbar: { left: 'prev,next today', center: 'title', right: 'dayGridMonth,timeGridWeek,listWeek' }, height: '100%', events: [], windowResize: () => { if (window.innerWidth < 768 && calendarView.style.display !== 'none') showListBtn.click(); }, editable: true, eventDrop: async (info) => { const taskId = info.event.id; const delta = info.delta; const mainTask = tasks.find(t => t.id === taskId); if (!mainTask) { info.revert(); return; } const moveDate = (dateStr, delta) => { const d = new Date(dateStr); d.setDate(d.getDate() + (delta.days || 0)); d.setMonth(d.getMonth() + (delta.months || 0)); d.setFullYear(d.getFullYear() + (delta.years || 0)); return d.toISOString().split('T')[0]; }; mainTask.startDate = moveDate(mainTask.startDate, delta); mainTask.endDate = moveDate(mainTask.endDate, delta); const children = tasks.filter(t => t.parentId === taskId); children.forEach(child => { child.startDate = moveDate(child.startDate, delta); child.endDate = moveDate(child.endDate, delta); }); await saveData(); renderAll(); } }); calendar.render(); };
...
const renderTaskList = () => { if (!taskListContainer) return; taskListContainer.innerHTML = ''; const tasksByParent = tasks.reduce((acc, task) => { const parentId = task.parentId || 'root'; if (!acc[parentId]) acc[parentId] = []; acc[parentId].push(task); return acc; }, {}); const ICONS = { toggle: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>` }; const renderTaskRecursive = (parentId) => { const childTasks = (tasksByParent[parentId] || []).sort((a,b) => { if (a.completed !== b.completed) return a.completed ? 1 : -1; return new Date(a.startDate) - new Date(b.startDate); }); childTasks.forEach(task => { const hasChildren = tasksByParent[task.id] && tasksByParent[task.id].length > 0; const card = document.createElement('div'); card.className = `task-card ${task.completed ? 'completed' : ''}`; if (task.parentId) card.classList.add('is-child'); if (task.isCollapsed) card.classList.add('children-collapsed'); card.dataset.id = task.id; const highestPrioLabel = getHighestPriorityLabel(task); if (highestPrioLabel && !task.completed) card.style.borderLeftColor = highestPrioLabel.color; const dateString = task.startDate === task.endDate ? task.startDate : `${task.startDate} ~ ${task.endDate}`; const labelsHtml = (task.labelIds || []).map(labelId => labels.find(l => l.id === labelId)).filter(Boolean).map(label => `<span class="task-label-badge" style="background-color: ${label.color}">${label.name}</span>`).join(''); const toggleBtnHtml = hasChildren ? `<button class="toggle-children-btn" data-action="toggle-children">${ICONS.toggle}</button>` : '<div style="width:24px; flex-shrink:0;"></div>'; card.innerHTML = `<div class="task-header">${toggleBtnHtml}<p class="task-text" title="ID: ${task.id}">${task.text.replace(/</g, "<").replace(/>/g, ">")}</p><div class="task-labels">${labelsHtml}</div></div><div class="task-footer"><span class="task-due-date">期間: ${dateString}</span><div class="task-actions"><button class="set-parent-btn" data-action="set-parent">親子設定</button><button class="edit-labels-btn" data-action="edit-labels">ラベル</button><button class="complete-btn" data-action="complete">${task.completed ? '未完了' : '完了'}</button><button class="delete-btn" data-action="delete">削除</button></div></div>`; taskListContainer.appendChild(card); if (hasChildren && !task.isCollapsed) { renderTaskRecursive(task.id); } }); }; renderTaskRecursive('root'); };
...
const showUndoBar = (deletedCount) => {
    ...
    if (undoBtn) {
        ...
        undoBtn.addEventListener('click', async () => {
            tasks.push(...lastDeletedTasks);
            lastDeletedTasks = [];
            undoBar.style.bottom = '-100px';
            clearTimeout(undoTimeout);
            renderAll();
            await saveData();
        }, { once: true }); // イベントリスナーが重複しないようにonceオプションを使用
    }
...
const addEventListeners = () => {
    if(taskForm) taskForm.addEventListener('submit', async (e) => { e.preventDefault(); const text = taskInput.value.trim(); const dates = taskDueDateInput._flatpickr.selectedDates; if (text && dates.length > 0) { const startDate = dates[0].toISOString().split('T')[0]; const endDate = dates.length > 1 ? dates[1].toISOString().split('T')[0] : startDate; tasks.push({ id: Date.now().toString(), text, startDate, endDate, completed: false, labelIds: [], parentId: null, isCollapsed: false }); taskForm.reset(); taskDueDateInput._flatpickr.clear(); renderAll(); await saveData(); } });
    if(taskListContainer) taskListContainer.addEventListener('click', async (e) => { const button = e.target.closest('button[data-action]'); if (!button) return; const card = e.target.closest('.task-card'); const taskId = card ? card.dataset.id : null; if (!taskId) return; const action = button.dataset.action; const task = tasks.find(t => t.id === taskId); if (!task) return; let needsSave = false; let needsRender = true; switch (action) { case 'toggle-children': task.isCollapsed = !task.isCollapsed; needsSave = true; break; case 'set-parent': openParentModal(taskId); needsRender = false; break; case 'edit-labels': openLabelModal(taskId); needsRender = false; break; case 'complete': task.completed = !task.completed; needsSave = true; break; case 'delete': const idsToDelete = [taskId, ...getDescendants(taskId)]; lastDeletedTasks = tasks.filter(t => idsToDelete.includes(t.id)); tasks = tasks.filter(t => !idsToDelete.includes(t.id)); showUndoBar(lastDeletedTasks.length); needsSave = true; break; } if (needsRender) renderAll(); if (needsSave) await saveData(); });
    if(showListBtn) showListBtn.addEventListener('click', () => { if (listView) listView.style.display = 'block'; if(calendarView) calendarView.style.display = 'none'; showListBtn.classList.add('active'); showCalendarBtn.classList.remove('active'); });
    if(showCalendarBtn) showCalendarBtn.addEventListener('click', () => { if (listView) listView.style.display = 'none'; if(calendarView) calendarView.style.display = 'block'; showListBtn.classList.remove('active'); showCalendarBtn.classList.add('active'); if(calendar) calendar.render(); });
    ...
    if (geminiTriggerBtn) geminiTriggerBtn.addEventListener('click', async () => { const promptText = geminiPrompt.value.trim(); if (!promptText || !GEMINI_API_KEY) { alert("プロンプトを入力するか、GeminiのAPIキーを設定してください。"); return; } geminiTriggerBtn.disabled = true; geminiTriggerBtn.querySelector('.default-text').style.display = 'none'; geminiTriggerBtn.querySelector('.loading-indicator').style.display = 'flex'; try { const existingTasksList = tasks.map(t => `- id: ${t.id}, text: ${t.text}`).join('\n'); const fullPrompt = `あなたは優秀なプロジェクトマネージャーです。ユーザーの指示に従い、タスクを生成・編集してください。\n\n# 既存タスク一覧\n${existingTasksList}\n\n# 出力フォーマット\n必ず以下のキーを持つJSONオブジェクトを、\`\`\`json ... \`\`\` の中に記述して出力してください。\n- "action": (string) "create" | "add" | "subdivide" のいずれか。\n- "targetTaskId": (string or null) "add"または"subdivide"アクションの対象となる既存タスクのID。\n- "tasks": (array) 生成または編集されたタスクオブジェクトの配列。各オブジェクトは以下のキーを持つ。\n  - "tempId": (string) このタスクの一時的なID。必ずユニークな文字列にしてください (例: "t1", "t2")。\n  - "text": (string) タスクの内容です。\n  - "startDate": (string) "YYYY-MM-DD"形式の開始日です。\n  - "endDate": (string) "YYYY-MM-DD"形式の終了日です。\n  - "parentId": (string or null) 親タスクの"tempId"です。親がない場合はnullにしてください。\n\n# アクションルール\n1.  **action: "create"**\n    - 新しいタスク群をゼロから生成します。\n    - 指示に「追加」「分割」といったキーワードがない場合のデフォルトアクションです。\n2.  **action: "add"**\n    - ユーザーが既存のタスクIDを指定し、「〜に...を追加して」のように指示した場合に使用します。\n    - "targetTaskId"に指定されたIDを親として、新しい子タスクを生成します。\n3.  **action: "subdivide"**\n    - ユーザーが既存のタスクIDを指定し、「〜を...に分割して」のように指示した場合に使用します。\n    - "targetTaskId"のタスクを親とし、その期間内で日々の具体的なサブタスクに分割します。\n    - サブタスク名は「(元のタスク名) - 1日目」のように命名してください。\n\n# 一般ルール\n- **スケジューリング**: 論理的な順序で、週末を避けつつ、期間内にタスクを割り振ってください。\n- **日付**: 今日の日付は ${new Date().toISOString().split('T')[0]} です。相対的な日付はこの日付を基準に計算してください。\n\n# ユーザーの入力\n${promptText}`; const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=${GEMINI_API_KEY}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts: [{ text: fullPrompt }] }] }) }); if (!response.ok) throw new Error(`Gemini APIエラー: ${response.status} ${await response.text()}`); const data = await response.json(); let jsonString = data.candidates[0].content.parts[0].text; const jsonMatch = jsonString.match(/```json([\s\S]*?)```/); jsonString = jsonMatch ? jsonMatch[1].trim() : jsonString.substring(jsonString.indexOf('{'), jsonString.lastIndexOf('}') + 1); const aiResponse = JSON.parse(jsonString); const idMap = {}; aiResponse.tasks.forEach(task => { if (task.tempId) { idMap[task.tempId] = Date.now().toString() + Math.random().toString(36).substr(2, 9); } }); const newTasks = aiResponse.tasks.map(task => { let parentId = task.parentId ? idMap[task.parentId] : null; if (aiResponse.action === 'add' || aiResponse.action === 'subdivide') { if (!task.parentId) { parentId = aiResponse.targetTaskId; } } return { id: idMap[task.tempId] || Date.now().toString() + Math.random().toString(36).substr(2, 9), text: task.text, startDate: task.startDate, endDate: task.endDate || task.startDate, parentId, completed: false, labelIds: [], isCollapsed: false }; }); tasks.push(...newTasks); renderAll(); await saveData(); geminiPrompt.value = ''; } catch (error) { console.error("Gemini処理エラー:", error); alert("タスクの自動生成に失敗しました。コンソールでエラー内容を確認してください。"); } finally { geminiTriggerBtn.disabled = false; geminiTriggerBtn.querySelector('.default-text').style.display = 'inline'; geminiTriggerBtn.querySelector('.loading-indicator').style.display = 'none'; } });
};
