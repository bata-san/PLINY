...
geminiTriggerBtn.addEventListener('click', async () => {
    ...
    geminiTriggerBtn.querySelector('.loading-text').style.display = 'inline';

    try {
        const fullPrompt = `以下の文章からタスクを抽出し、JSON形式の配列で出力してください。各タスクにはtext, startDate, endDateのキーを含めてください。日付はYYYY-MM-DD形式で。今日の日付は${new Date().toISOString().split('T')[0]}です。\n\n---\n\n${promptText}`;
        ...
        newTasks.forEach(task => {
            tasks.push({ id: Date.now().toString() + Math.random(), text: task.text, startDate: task.startDate, endDate: task.endDate, completed: false });
        });
        ...
        geminiTriggerBtn.querySelector('.loading-text').style.display = 'none';
    }
});
